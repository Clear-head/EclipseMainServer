...
def _get_content_prompt(user_detail: str, people_count: int, people_exclusion_rule: str) -> str:
    ...
    return f"""
        ...
        **콘텐츠 관련 키워드 참고 카테고리 (사용자가 직접 언급한 경우에만 추출):**
        ...
        4. 시간대 (낮, 저녁, 밤, 주말, 평일 등 - 사용자가 직접 언급한 경우만)
...
VALIDATION_PROMPT = """
[역할]
당신은 사용자 입력이 {category} 추천을 위한 의미있는 정보인지 판단하고, 
랜덤 추천을 원하는지 구분하는 전문가입니다.
...
🔹 **1. 랜덤 추천 요청 여부 판단 (매우 엄격하게 판단)**

**⚠️ 중요: 랜덤 추천은 매우 제한적으로만 인정합니다**

다음 **모든 조건**을 동시에 만족해야만 "랜덤 추천"으로 판단:
1. **선택 회피 표현만 있어야 함**: "아무거나", "상관없어", "뭐든지", "알아서", "편한대로" 등
2. **구체적인 키워드가 전혀 없어야 함** (형용사, 명사, 맛 표현, 분위기 등 모두 없어야 함)
3. **부정 표현도 없어야 함** ("~싫어", "~말고" 등 없어야 함)
4. **{category}와 관련된 어떤 힌트도 없어야 함**
...
**랜덤 추천 X (일반 추천으로 처리):**
- "가볍게 먹고싶어" → 일반 추천 (**"가볍게"**라는 명확한 키워드 존재)
- "아무거나 매운거" → 일반 추천 (**"매운"**이라는 키워드 존재)
- "그냥 단거" → 일반 추천 (**"달콤한"**이라는 키워드 존재)
- "상관없어 파스타" → 일반 추천 (**"파스타"**라는 메뉴 존재)
- "아무거나 조용한 곳" → 일반 추천 (**"조용한"**이라는 분위기 키워드 존재)
- "매운거 싫어" → 일반 추천 (부정 표현 = 명확한 선호)
- "시끄러운 곳 말고" → 일반 추천 (부정 표현 = 명확한 선호)

🔹 **2. 의미있는 입력 여부 판단 (랜덤 아닌 경우에만)**

**의미있는 입력** (다음 중 하나라도 해당):
1. {category}와 직접적으로 관련된 구체적인 선호사항 포함
2. {category}의 분위기, 스타일, 특징을 표현한 경우
3. **맛/분위기/느낌을 표현하는 형용사** 포함
   (예: "힙한", "모던한", "아늑한", "강렬한", "매운", "달콤한", **"가볍게"**, "담백한" 등)
4. 부정 표현으로 회피하고 싶은 것을 명확히 표현한 경우 
   (예: "매운거 싫어", "시끄러운 곳 말고", "싱거운거 싫어")
5. **메뉴나 음식 종류 언급** (예: "파스타", "김치찌개", "삼겹살" 등)
6. **활동이나 용도 언급** (예: "공부", "데이트", "회의" 등)

**카테고리별 관련 정보 예시:**
- 카페: 메뉴(아메리카노, 라떼, 디저트, 케이크 등), 분위기(조용한, 아늑한, 힙한, 모던한 등), 용도(공부, 업무 등), 시설(와이파이, 콘센트 등)
- 음식점: 메뉴(파스타, 삼겹살, 김치찌개 등), 맛/특징(**가볍게**, 얼큰한, 담백한, 매운, 순한, 짭짤한 등), 분위기, 가격대
- 콘텐츠: 활동 종류(영화, 전시회, 공연 등), 장르(액션, 로맨스 등), 분위기

**의미없는 입력** (다음 중 하나라도 해당):
1. **{category}와 전혀 무관한 발화** 
   - 예: 음악 재생 명령 ("아무노래나 틀어", "음악 켜줘")
   - 예: 다른 카테고리 언급 (카페를 물었는데 음식점 메뉴만 나열)
2. **메타 표현**: "테스트", "없음", "ㅋㅋㅋ", "ㅎㅎㅎ"
3. **일반적인 인사나 질문**: "안녕하세요", "뭐가 있어요?"
4. **무작위 자판 입력**: "asdf", "ㅁㄴㅇㄹ"
