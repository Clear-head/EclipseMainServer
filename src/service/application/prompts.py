...
🔹 **1. 랜덤 추천 요청 여부 판단**
다음 경우 "랜덤 추천"으로 판단:
- "아무거나", "상관없어", "뭐든", "알아서", "편한대로" 등 **선택 회피 표현만** 있는 경우
- 구체적인 선호사항이 전혀 없는 경우

다음 경우 "일반 추천"으로 판단:
- 선택 회피 표현 + 구체적 키워드가 함께 있는 경우
  예시:
  - "아무거나 매운거" → 일반 추천 (키워드: "매운")
  - "그냥 단거" → 일반 추천 (키워드: "달콤한")
  - "상관없어 파스타" → 일반 추천 (키워드: "파스타")
  - "아무거나 조용한 곳" → 일반 추천 (키워드: "조용한")
- 부정 표현이 포함된 경우 (명확한 선호)
  예시: "매운거 싫어", "시끄러운 곳 말고"
...
3. 형용사나 감성적 표현으로 원하는 느낌을 전달한 경우 
   (예: "힙한", "모던한", "아늑한", "강렬한", "매운", "달콤한" 등)
...
- 음식점: 메뉴(파스타, 삼겹살, 김치찌개 등), 맛/특징(얼큰한, 담백한, 매운, 순한, 짭짤한 등), 분위기, 가격대
- 콘텐츠: 활동 종류(영화, 전시회, 공연 등), 장르(액션, 로맨스 등), 분위기

**의미없는 입력** (다음 중 하나라도 해당):
1. **명확한 선택 회피 + 구체적 정보 없음**: "아무거나", "몰라", "상관없어", "그냥"만 있는 경우
