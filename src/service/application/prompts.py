...
def get_category_prompt(category: str, user_detail: str, people_count: int) -> str:
    ...
    category_prompts = {
        ...
        **제외할 키워드:**
        ...
        이 문장에서 음식점 활동과 관련된 핵심 키워드를 정확히 2개 이상 추출해서 쉼표로 구분해서 알려줘.
        사용자가 음식명만 입력했을 경우에는 음식명만 제공해줘.

        **음식점 관련 키워드 우선순위:**
        1. 메뉴 (김치찌개, 파스타, 초밥, 스테이크, 돼지고기, 삼겹살 등 구체적인 음식명)
        2. 메뉴의 재료(김치, 파스타, 회, 고기, 마라 등 음식의 재료명)
        3. 맛/특징 (얼큰한, 담백한, 신선한, 진한, 부드러운, 매운 등)
        4. 분위기 (데이트, 가족, 친구, 단체, 조용한 등)
        5. 가격대 (저렴한, 보통, 비싼, 고급 등)
        6. 시간대/상황 (아침, 점심, 저녁, 야식, 회식 등)
...
[판단 기준]
의미있는 입력:
- 장소, 음식, 분위기, 특징 등 구체적인 선호사항
- 예: "조용한 곳", "파스타", "뷰가 좋은", "디저트 맛집"

의미없는 입력:
- "아무거나", "몰라", "글쎄", "상관없어" 등 선택을 회피하는 표현
- "그냥", "막", "뭐든지" 등 애매한 표현만 있는 경우
- 구체적인 정보가 전혀 없는 경우

[출력]
다음 중 하나만 출력하세요:
- "의미있음" (구체적인 선호사항이 포함된 경우)
- "의미없음" (선택을 회피하거나 애매한 표현만 있는 경우)
"""
