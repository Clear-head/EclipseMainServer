...
def _get_restaurant_prompt(user_detail: str, people_count: int, people_exclusion_rule: str) -> str:
    ...
    return f"""
        ...
        **부정 표현 처리 규칙:**
        - **명확한 부정 표현이 있을 때만** 반대 개념으로 변환합니다.
        - 부정 표현: "~싫어", "~안돼", "~빼고", "~말고", "~하지 마", "~제외", "안 ~", "~없이"
        - **중요**: 사용자가 단순히 키워드만 입력했을 경우(예: "매운거", "짠거") 그대로 추출하고 반대 개념으로 변환하지 마세요.

        **부정 표현 변환 예시 (음식점):**
        - "매운게 싫어" → "순한", "담백한" (명확한 부정 표현 '싫어' 포함)
        - "매운거" → "매운" (부정 표현 없음, 그대로 추출)
        - "기름진 거 빼고" → "담백한", "깔끔한" (명확한 부정 표현 '빼고' 포함)
        - "기름진" → "기름진" (부정 표현 없음, 그대로 추출)
        - "짜지 않은" → "적당한 간", "싱거운" (명확한 부정 표현 '않은' 포함)
        - "짠거" → "짭짤한" (부정 표현 없음, 그대로 추출)
        - "싱거운거 싫어" → "짭짤한", "간이 센" (명확한 부정 표현 '싫어' 포함)
        - "느끼한 거 말고" → "산뜻한", "담백한" (명확한 부정 표현 '말고' 포함)

        **절대 규칙: 부정 표현이 명확히 없으면 사용자 입력을 그대로 키워드로 추출하세요.**

        이 문장에서 음식점 활동과 관련된 핵심 키워드를 추출해서 쉼표로 구분해서 알려줘.

        **음식점 관련 키워드 참고 카테고리 (사용자가 직접 언급한 경우에만 추출):**
        1. 메뉴 (김치찌개, 파스타, 초밥, 스테이크, 돼지고기, 삼겹살, 마라탕 등)
        2. 사용자가 명시적으로 언급한 재료
        3. 맛/특징 (얼큰한, 담백한, 신선한, 진한, 부드러운, 매운, 순한, 짭짤한, 싱거운 등)
        4. 분위기 (데이트, 가족, 친구, 단체, 조용한 등)
        5. 가격대 (저렴한, 보통, 비싼, 고급 등)
